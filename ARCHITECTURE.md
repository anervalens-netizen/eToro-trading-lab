...
4. La fiecare bară nouă eligibilă se construiește un packet sanitizat. Runnerul Dell folosește loginul ChatGPT existent și pornește `gpt-5.6-sol` stateless într-o unitate systemd: numai auth-ul ChatGPT este montat read-only, home/SSH sunt mascate și execuția altor binare este interzisă. Sol decide `OPEN`, `CLOSE` sau `HOLD` pentru masterul de 1.000 USD. La `OPEN` poate selecta un candidat sau genera direct un intent în catalog și limitele transmise. Decizia este hash-bound, expiră și se consumă o singură dată.
...
10. Dashboard-ul light arată master NAV/P&L, cele 12 ledgere, istoric paginat, lifecycle, review-uri, propuneri și consum AI. Authentik și verificarea exactă a ownerului rămân obligatorii.
...
Masterul de 1.000 USD primește candidații strategiilor și intenții directe Sol pentru instrumentele deschise/valide din catalog. Eligibility și cost preview eToro decid compatibilitatea exactă înainte de propunere. Configurația DEMO permite maximum 500 USD per ordin, 1.000 USD gross, 20 USD risc proiectat/trade, 25 USD pierdere zilnică și 50 USD lunară, fără leverage. Pentru buy nelevierat pe acțiuni, `settlementType=real` înseamnă activ suport, nu cont REAL; ruta și credentialul rămân strict DEMO. Aceste limite nu se transferă automat la REAL.
