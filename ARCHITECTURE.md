...
7. Pentru un write eToro DEMO, executorul verifică seal-ul public, aprobarea exactă one-time, cheia separată creată în eToro pentru mediul DEMO fără scope REAL/alt write, broker truth și eligibility/cost/quote live. Kill blochează opens, dar permite numai close reduce-only sigilat. După ACK, reconcilierea eșuată activează kill.
...
- Cheia largă a ownerului nu este acceptată de executor. Acesta cere un User Key separat, generat în eToro cu Environment=Demo și Permission=Write, și refuză orice scope REAL sau alt scope de write.
