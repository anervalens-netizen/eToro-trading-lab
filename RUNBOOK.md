...
Fără `ENABLE_V2_DEMO_EXECUTION`, numai `etoro-v2-decision-apply.service` este permis: nu are rețea, broker sau signer și înregistrează `broker_write=false`, fără `OrderCommand`. Gate-ul inversează această graniță: shadow applier-ul se oprește, iar `etoro-v2-decision-apply-execution.service` poate transforma numai selecția unui candidat determinist în intent. Modelul nu poate inventa symbol, side, amount, SL/TP, horizon sau slippage.
