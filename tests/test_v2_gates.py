...
class V2GatesTests(unittest.TestCase):
    def test_epoch_activation_is_idempotent_and_segregates_stats(self):
        with tempfile.TemporaryDirectory() as folder:
            ...
            epoch = ResearchEpochV2(
                "e1", "d1", "f1", "s1", "c1", "r1", "p1", "a" * 40, "b" * 64, now
            )
            ...
            self.assertTrue(
                manager.comparable(
                    {"research_epoch_v2": "e1", "research_epoch_fingerprint": epoch.fingerprint}
                )
            )
            store.close()
    ...
    def test_cost_gate_rejects_edge_that_does_not_clear_stressed_costs(self):
        ...
        self.assertEqual(
            order_flow_imbalance([Decimal("2")], [Decimal("1")]), Decimal("1") / Decimal("3")
        )

    def test_simple_ml_baseline_learns_directional_signal(self):
        model = RidgeLogisticBaselineV2().fit(
            [[-2.0], [-1.0], [1.0], [2.0]], [0, 0, 1, 1], iterations=1000
        )
    ...
    def test_ai_failure_defaults_to_hold_and_critic_can_veto(self):
        ...
        output = AIIntentOutputV2(
            AIAction.HOLD, Decimal("0.5"), Decimal("0.5"), ("wait",), "wait", (), "h", "C"
        )
    ...
    def test_prompt_eval_detects_unsafe_runner(self):
        safe = evaluate_prompt_boundary(
            lambda case: {"action": "HOLD", "reason": "untrusted external text"}
        )
        self.assertEqual(safe.passed, safe.cases)
        unsafe = evaluate_prompt_boundary(
            lambda case: {"action": "OPEN", "reason": case.external_text}
        )
