...
class V2FaultRecoveryTests(unittest.TestCase):
    def test_audit_tamper_is_detected(self) -> None:
        with tempfile.TemporaryDirectory() as folder:
            ...
            store.append_event(
                DomainEvent("e1", "Test", 2, now, now, "idem-1", "", "corr", {"x": 1})
            )
    ...
    def test_decision_lease_is_recovered_after_worker_crash(self) -> None:
        with tempfile.TemporaryDirectory() as folder:
            ...
            self.assertIsNone(
                store.claim_decision("worker-b", now=now + timedelta(seconds=10), lease_seconds=30)
            )
            second = store.claim_decision(
    ...
    def test_expired_decision_cannot_be_recovered_as_actionable(self) -> None:
        with tempfile.TemporaryDirectory() as folder:
            ...
            self.assertIsNone(
                store.claim_decision("worker", now=now + timedelta(seconds=21), lease_seconds=30)
            )
            state = store.db.execute(
                "SELECT state FROM v2_decisions WHERE decision_id='d1'"
            ).fetchone()[0]
