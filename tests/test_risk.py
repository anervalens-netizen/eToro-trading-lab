...
def intent() -> TradeIntent:
    return TradeIntent(
        "BTC", Side.BUY, Decimal("50"), Decimal("0.8"), "test", Decimal("0.03"), Decimal("0.06"), 1
    )
...
class RiskTests(unittest.TestCase):
    def test_persisted_keypair_keeps_private_key_out_of_verifier(self) -> None:
        with tempfile.TemporaryDirectory() as folder:
            ...
            engine = DeterministicRiskEngine(limits(), load_private_signing_key(private_path))
            order = engine.evaluate(intent(), context()).order
            verifier = OrderVerifier(limits(), load_public_verifying_key(public_path))
    ...
    def test_limits_fail_closed(self) -> None:
        cases = [
            (
                dataclasses.replace(intent(), amount_usd=Decimal("101")),
                context(),
    ...
    def test_executor_rejects_a_sealed_order_after_quote_age_limit(self) -> None:
        engine = DeterministicRiskEngine(limits(), b"x" * 32)
        order = engine.evaluate(intent(), context(evaluated_at=1000, quote_observed_at=1000)).order
