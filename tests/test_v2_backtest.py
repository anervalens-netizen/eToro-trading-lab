...
class V2BacktestTests(unittest.TestCase):
    def test_kernel_backtest_executes_next_bar_and_closes_through_exit_evaluator(self) -> None:
        ...
        bars = [
            HistoricalBar(start + timedelta(minutes=15 * i), Decimal(str(100 + i)), Decimal(str(101 + i)), Decimal(str(99 + i)), Decimal(str(100 + i)))
            for i in range(8)
        ]
        mandate = CapitalMandate(
            frozenset({"AAPL"}), Decimal("500"), Decimal("20"), Decimal("1000"),
            Decimal("1000"), 1, Decimal("100"), Decimal("100"), Decimal("100"),
            Decimal("0.20"), Decimal("0.30"), 60, Decimal("100"), Decimal("500"),
        )
        ...
        def signal(index, history, bid, ask, snapshot_hash):
            ...
            return IntentEnvelope(
                "bt-intent", "master", "A", "baseline", "2", "AAPL", Side.BUY,
                Decimal("100"), Decimal("0.8"), Decimal("0.6"), Decimal("0.02"),
                Decimal("0.03"), 3600, now, now, now + timedelta(hours=1), bid, ask,
                Decimal("500"), Decimal("50"), snapshot_hash, correlation_id="bt",
            )
