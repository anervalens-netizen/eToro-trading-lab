    ...
    def claim(
        ...
        *,
        ...
        with self.store.transaction() as cursor:
            ...
            cursor.execute(
                "UPDATE v2_ai_packets SET state='EXPIRED',updated_at=%s WHERE state IN ('PENDING','ERROR') AND expires_at<%s",
                (current, current),
            )
    ...
    def fail(
        ...
        *,
        ...
        with self.store.transaction() as cursor:
            ...
            cursor.execute(
                "UPDATE v2_ai_packets SET state=%s,claim_token=NULL,lease_expires_at=NULL,updated_at=%s WHERE packet_id=%s",
