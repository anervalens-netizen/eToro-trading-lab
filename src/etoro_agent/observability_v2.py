...
class HealthEvaluatorV2:
    def evaluate(
        ...
        *,
        ...
        age = None if data_observed_at is None else max(0.0, (current - data_observed_at.astimezone(timezone.utc)).total_seconds())
        ...
        if unknown_orders:
            oldest_unknown = max(
                0.0,
                max((current - value.astimezone(timezone.utc)).total_seconds() for value in unknown_orders.values()),
            )
            signals.append(HealthSignal("orders_unknown", "HALT_NEW", f"{len(unknown_orders)} unresolved order(s)"))
        if reconcile_delta_usd != 0:
            signals.append(HealthSignal("reconciliation", "HALT_NEW", f"delta={reconcile_delta_usd}"))
        ...
        if last_backup_at is None or current - last_backup_at.astimezone(timezone.utc) > backup_max_age:
            signals.append(HealthSignal("backup", "DEGRADED", "backup overdue"))
        if last_anchor_at is None or current - last_anchor_at.astimezone(timezone.utc) > anchor_max_age:
            signals.append(HealthSignal("audit_anchor", "HALT_NEW", "audit anchor overdue"))
        return RuntimeMetricsV2(
            current.isoformat(), age, websocket_reconnects, len(unknown_orders), oldest_unknown,
            reconcile_delta_usd, failure_rate, audit_chain_valid,
            None if last_backup_at is None else last_backup_at.astimezone(timezone.utc).isoformat(),
            None if last_anchor_at is None else last_anchor_at.astimezone(timezone.utc).isoformat(),
            tuple(signals),
        )
