...
class DashboardService:
    ...
    def _read_heartbeats(connection: sqlite3.Connection, tables: set[str]) -> list[dict[str, Any]]:
        ...
        for row in connection.execute(
            ...
            recorded_at = str(row["recorded_at"])
            max_age = _HEARTBEAT_MAX_AGE_SECONDS.get(
                service, _DEFAULT_HEARTBEAT_MAX_AGE_SECONDS
            )
            try:
                ...
                age_seconds: int | None = max(
                    0,
                    round((now - recorded.astimezone(timezone.utc)).total_seconds()),
                )
            except ValueError:
                age_seconds = None
    ...
    def snapshot(self) -> dict[str, Any]:
        ...
        ai_review_unhealthy = bool(
            ai.get("review_errors", 0) or ai.get("review_stale_claims", 0)
        )
        health_status = (
            "halted"
            if kill_active
