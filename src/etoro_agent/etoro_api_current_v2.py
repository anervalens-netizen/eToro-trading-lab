...
class EtoroPublicApiDemoClientV2:
    ...
    def _request(
        ...
        *,
        ...
        request = urllib.request.Request(url, data=encoded, headers=self._headers(req_id), method=method)
        try:
    ...
    def cost_preview(self, order_body: Mapping[str, Any]) -> ApiResponse:
        allowed = {
            "action", "transaction", "symbol", "instrumentId", "settlementType",
            "orderType", "leverage", "amount", "orderCurrency", "positionIds",
        }
        body = {key: value for key, value in order_body.items() if key in allowed and value is not None}
    ...
    def _select_configuration(
        ...
        *,
        ...
        configs = [
            item for item in row.get("leverageConfigs", [])
        ...
        config = config or next((item for item in configs if item.get("settlementType") == "cfd"), None)
    ...
    @staticmethod
    def _validate_stop_take(config: Mapping[str, Any], entry: Decimal, stop: Decimal, take: Decimal) -> None:
        stop_pct = abs(entry - stop) / entry * Decimal("100")
        take_pct = abs(take - entry) / entry * Decimal("100")
        if not Decimal(str(config["minStopLossPercentage"])) <= stop_pct <= Decimal(str(config["maxStopLossPercentage"])):
            raise PermissionError("stop-loss is outside broker bounds")
        if not Decimal(str(config["minTakeProfitPercentage"])) <= take_pct <= Decimal(str(config["maxTakeProfitPercentage"])):
    ...
    def open_by_amount(
        ...
        *,
        ...
        preview = self.cost_preview({
            "action": "open", "transaction": transaction, "instrumentId": instrument_id,
            "settlementType": settlement, "orderType": "mkt", "leverage": leverage,
            "amount": float(amount_usd), "orderCurrency": "usd",
        })
        if not preview.ok:
        ...
        return self._request(
            ...
            DEMO_CREATE_ORDER,
            body={
                "action": "open", "transaction": transaction, "symbol": None,
                "instrumentId": instrument_id, "settlementType": settlement,
                "orderType": "mkt", "triggerRate": None, "leverage": leverage,
                "amount": float(amount_usd), "orderCurrency": "usd", "units": None,
                "contracts": None, "stopLossRate": float(stop_loss_rate),
                "takeProfitRate": float(take_profit_rate), "stopLossType": "fixed",
                "additionalMargin": None, "positionIds": None,
            },
    ...
    def cash_truth(self) -> DemoCashTruth:
        ...
        manual = sum(
            (Decimal(str(item.get("amount", 0))) for item in open_orders if isinstance(item, dict) and int(item.get("mirrorID", item.get("mirrorId", 0)) or 0) == 0),
            Decimal("0"),
        )
        pending = sum(
            (Decimal(str(item.get("amount", 0))) for item in pending_orders if isinstance(item, dict)),
            Decimal("0"),
        )
