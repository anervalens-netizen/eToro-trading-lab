...
def market_is_open(instrument: InstrumentSpec, at: datetime) -> bool:
    """Versioned broker-calendar gate; missing/stale/unknown always means closed."""

    path = os.getenv("ETORO_V2_MARKET_CALENDAR_FILE", "config/market-calendar-v2.json")
    try:
        return load_market_calendar_release(path).is_open(instrument.symbol, at)
...
class MarketDataCollector:
    def __init__(self, client: EtoroPublicApiDemoClientV2) -> None:
        self.client = client
    ...
    def collect(
        ...
        *,
        ...
        rates = self.client.rates((instrument_id,))
        candles = self.client.history_candles(
            instrument_id=instrument_id,
            interval=interval,
            count=min(count + 1, 1000),
        )
        ...
        quality = validate_candles(
            cast(tuple[CandleLike, ...], parsed_candles),
            interval,
            now=captured_at,
