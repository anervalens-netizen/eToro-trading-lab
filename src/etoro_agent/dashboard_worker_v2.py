...
from .dashboard_v2 import DashboardServiceV2, create_v2_app


def build_app(runtime: str, config: str) -> Any:
    from fastapi import Request
    from fastapi.responses import JSONResponse

    app = create_v2_app(DashboardServiceV2(runtime, config))
    ...
    async def owner_boundary(request: Request, call_next: Any) -> Any:
        ...
        if request.url.path != "/healthz" and trusted_proxy and not hmac.compare_digest(client, trusted_proxy):
            ...
            response = JSONResponse({"detail": "proxy boundary authentication failed"}, status_code=403)
        elif request.url.path != "/healthz" and not hmac.compare_digest(
...
def main() -> None:
    ...
    uvicorn.run(build_app(args.runtime, args.config), uds=args.uds, access_log=False)
